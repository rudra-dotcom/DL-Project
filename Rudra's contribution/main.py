...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='/root/FastBaseline/data/imagenet', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR', 'IMNET', 'IMNETEE', 'FLOWERS', 'INAT', 'INAT19'],
    ...
    parser.add_argument('--fuse-bn-eval', action='store_true', default=False,
                        help='fuse Conv-BN blocks at evaluation time for RepViT models')
    return parser
...
def main(args):
    ...
    if args.log_wandb and wandb is None:
        raise RuntimeError('wandb is not installed but --log-wandb was requested')

    if utils.is_main_process() and not args.eval and use_wandb:
    ...
    if args.model.startswith('repvit_'):
        model_kwargs['distillation'] = (args.distillation_type != 'none')
    ...
    if args.finetune:
        ...
        checkpoint_model = _checkpoint_state_dict(checkpoint)
        state_dict = model.state_dict()
        for k in list(checkpoint_model.keys()):
            if k in state_dict and checkpoint_model[k].shape != state_dict[k].shape:
                print(f"Removing mismatched key {k} from pretrained checkpoint")
    ...
    if args.distillation_type != 'none':
        ...
        teacher_model.load_state_dict(_checkpoint_state_dict(checkpoint))
    ...
    if args.resume:
        ...
        msg = model_without_ddp.load_state_dict(_checkpoint_state_dict(checkpoint), strict=True)
        print(msg)
    ...
    if args.eval:
        if args.fuse_bn_eval and args.model.startswith('repvit_'):
